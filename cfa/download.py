...
def standings():
    ...
    def process_contest(c, problems, rows):
        data = []
        for p in problems:
            data.append(dict(
                name=p['name'],
    ...
    for c in Contest.select():
        ...
        with models.db.atomic():
            process_contest(c, problems, rows)


def hacks():
    def process_contest(c, hacks):
        unrated_skipped = 0
        data = []
        for h in hacks:
...
def submissions(h5_path):
    ...
    def process_batch(c, subs, last_batch_ids):
        # Subs may be repeated if someone has made a new submission since the last batch was
        # processed. Batches are newest to oldest so continue processing ignoring the new sub, we're
        # not skipping any sub in the middle so it's fine.
    ...
    def do_contest(c):
        ...
        # make the program crash on systems with very limited RAM.
        # Try in batches
        batch_size = 100_000
        ...
        while True:
            ...
            last_batch_ids = process_batch(c, subs, last_batch_ids)
    ...
    with tmodels.Table.open_append(h5_path) as table:
        ...
        for c in Contest.select():
            ctable = table.get_or_create_table(group, f'contest{c.id}', tmodels.Submission)
...
def move_subs_h5_to_db(h5_path):
    ...
    with tmodels.Table.open_read(h5_path) as table, models.db.atomic():
        group = table.root.submissions
        all_tables = [table for table in group if getattr(table.attrs, 'done', False)]
